...
<form id="orderForm" class="hide" action="/order/save" method="post">
		...
		<c:forEach items="${carMap }" var="cart" varStatus="status">
			...
			<input type="hidden" name="orderItems[${status.index}].totalFee" value="${cart.value.item.price * cart.value.num}"/>
			<input type="hidden" name="orderItems[${status.index}].title" value="${cart.value.item.title}"/>
			<input type="hidden" name="orderItems[${status.index}].picPath" value="${cart.value.item.image}"/> --%>
		</c:forEach>
		<%--订单总价--%>
		...
		<input type="hidden" name="receiverName" value="入云龙"/>
		<input type="hidden" name="receiverMobile" value="15891588888"/>
		<input type="hidden" name="receiverState" value="北京"/>
		<input type="hidden" name="receiverCity" value="北京"/>
		<input type="hidden" name="receiverDistrict" value="昌平区"/>
		<input type="hidden" name="receiverAddress" value="西三旗五一路21号"/>
		<input type="hidden" name="receiverZip" value="310001"/>
</form>
    ...
    <div class="goods-suit goods-last">
	 <c:forEach items="${carMap }" var="cart">
		<div class="goods-item goods-item-extra">
			...
			<div class="p-img">
				<a target="_blank" href="/item/${cart.value.item.id}.html">
					<img src="${cart.value.item.images[0]}" alt="">
				</a>
			</div>
			<div class="goods-msg">
				<div class="p-name">
					<a href="/item/${cart.value.item.id}.html" target="_blank">
						${cart.value.item.title }
					</a>
				</div>
				<div class="p-price">
					...
					<strong>￥<fmt:formatNumber
							...
							minFractionDigits="2" value="${cart.value.item.price / 100 }" /></strong>
					<!--增加预售金额显示 end-->
					<span class="ml20"> x${cart.value.num} </span>
